...
def avgby(tbl, col, by):
  if tbl == []:
    ...
    for i in range(len(tbl)):
      col_col.append(tbl[i][col])
      col_by.append(tbl[i][by])

    for j in range(len(col_by)):
      if col_by[j] not in result:
        result[col_by[j]] = col_col[j]
        key_count[col_by[j]] = 1
      else:
        result[col_by[j]] += col_col[j]
        key_count[col_by[j]] += 1

    for x in result:
      result[x] = result[x] / key_count[x]
    return result
